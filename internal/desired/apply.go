...
/*
 * Операторы SecLang «из файла» и их сокращения -- один список с контроллером
 * (compile/pack.ts): что тот собрал в data, то здесь переписывается на
 * абсолютный путь каталога профиля. Длинные имена в альтернации стоят раньше
 * коротких, иначе @pmFromFile срезался бы до @pmf с хвостом.
 */
var fromFile = regexp.MustCompile(`@(pmFromFile|pmf|ipMatchFromFile|ipMatchF)\s+([^"\s]+)`)

// Apply пишет манифест в dataDir/http и перечитывает реестр.
// Каталог dataDir сам не переименовываем: в образе он с нижнего слоя
// overlay, rename даёт EXDEV.
