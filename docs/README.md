...
outcomes:
  ...
  - on: deny
    ...
    ttl: 1h
  - on: overload              # очередь не ниже порога либо запрос снят на входе
    at: 60                    # порог заполнения очереди, 25..100; без него -- только сброс
    list: shed_clients
    ttl: 10m
```

Четыре триггера — три собственных вердикта этой фазы и перегрузка:
...
| `overload` | очередь при постановке запроса заполнена не ниже `at` процентов (25..100, без порога — 100) либо запрос снят из-за полной очереди (`MODSEC_QUEUE_LIMIT`) |

`overload` одинаков у всех инспекторов (`internal/overload`): ниже порога строка молчит; от порога до
края запрос проверяется как обычно и получает настоящий вердикт, а действие строки едет рядом; на
снятии из-за полной очереди срабатывают все строки перегрузки, и действие едет рядом с
`verdict: error`, где модуль исполнит только свои глаголы. На протухшем бюджете
(`MODSEC_DEADLINE_EXCEEDED`) строка молчит: дедлайн бывает и у короткой волны. Под перегрузкой
дешевле снять горячих клиентов записью в набор — дальше их режет локальный слой на краю, не грея
очередь, — чем гнать весь трафик через `wait` и дедлайны. Запись публикует сам инспектор, мимо
ответа; повод строки без своего кода — `MODSEC_QUEUE_LIMIT`. Строки перегрузки — только на фазе
запроса.
