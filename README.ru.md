...
Блокировка, не зависящая от счёта, настраивается отдельно: `WAF_MODSEC_DENY_RULE_IDS` и
...
Профили приходят от контроллера поколением: новый набор компилируется рядом с рабочим и подменяется
...
- Строка делает одно: `to` с `do` — сигнал соседу, `list` с `ttl` — запись в динамический набор. После
  ...
  и `asn` требуют справочник сетей (`WAF_MODSEC_GEO_ADDR`); молчащий справочник сетей — `error` с
...
| `MODSEC_GEO_UNAVAILABLE` | строка пишет сеть или систему, а справочник сетей молчит |
