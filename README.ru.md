...
модуль ──► waf.req.modsec ──►  modsec  ──► allow | score | deny
                                 ...
                                 └── просьбы соседей: масштаб счёта, пропуск проверки
```

## Как он судит

Каждое сообщение становится транзакцией движка. Движок работает в `SecRuleEngine DetectionOnly`:
инспектор сообщает уверенность, а отказывает nginx по `waf_score_deny` своей фазы. Аномальный счёт CRS
...
`WAF_MODSEC_DENY_TAGS` называют правила, срабатывание которых — отказ, а не вклад в счёт.
Вмешательство движка сопоставляется странице отказа через `status_map.yaml` (403 и 406 — `blocked`,
400 — `malformed`); код ответа из правила клиенту не уходит.

**Тело.** Тело крупнее порога инлайна приходит по локатору из обменника. Усечённое тело не
...
срок вышел), `resume=prefer` собирает транзакцию заново по снимку запроса, а `resume=require` отвечает
`error` с `MODSEC_RESUME_LOST`. В аудите оба пути различимы полем `resumed`.

**Кадры WebSocket** от клиента судятся синтетической транзакцией: POST на адрес рукопожатия с
...
Профили приезжают от контроллера поколением: новый набор компилируется рядом с боевым и подменяется
...
- Строка делает одно: `to` с `do` — просьба соседу, `list` с `ttl` — запись в живой набор. После
  отказа просьбы соседям не доезжают, а записи в наборы, маркеры и журнал работают. `net`, `net_all`
  и `asn` требуют кодер гео (`WAF_MODSEC_GEO_ADDR`); молчащий кодер — `error` с
  `MODSEC_GEO_UNAVAILABLE`.

Исход каждой доставленной просьбы (`applied` или `no_rule`) и значения `score_raw`,
